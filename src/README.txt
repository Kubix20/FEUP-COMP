...
CODE GENERATION: In the code generation phase, the AST and Symbol Table built are used to recursively pass through the program tree and build the final code. Whenever possible, optimizations are made in order to select the best instructions for each case. As per the locals and stack values in functions, the function's body is generated first, while updating helper variables that determine the optimal value for these variables. After generating the code for the body, we go back in the file and write the values in the appropriate position. For while loop, an optimal template is used (there are no unecessary goto operations). The main problem with the code generator is that it cannot perform optimizations such as constant propagation or lower the number of registers used through register allocation.
